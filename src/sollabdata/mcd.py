...
class MCD(AbsCD):
    ...
    def subtract(self, ref_id = -1, ys=None, ids_to_subtract=None, drop_zeros=True, same_temp=True, sub_once=True, ystd_col= None):
            ...
            for zi in range(len(zero_idx)):
                ...
                if same_temp:
                    ...
                    next_zero = (self.info_df.at[zero_idx[zi+1], self.labels['SCAN_NUM']]
                                 if zi+1 < len(zero_idx) else False)
                ...
                if sub_once:
                    sub_id = self.info_df.loc[(self.info_df['id'].isin(sub_id)) & (self.info_df['Subtracted'].isnull())]['id'].to_numpy()
        ...
        # if only want to subtract one zero off of selected
            ...
            if ystd_col is not None:
                ...
                super().add(ref_id=ref_id, ids_to_subtract=ids_to_subtract, ys=ystd_col, square=True)
    ...
    def subtract_component(self, component: AbsCD, ys=None, ystd_col=None, scalar:float = 1.0, inplace:bool=True):
        ...
        for i,row in self.info_df.iterrows():
            ...
            match_component = component_df.loc[(component_df[component.labels['TEMP']] == t) & (component_df[component.labels['FIELD']] == f)].sort_values(component.labels['SCAN_NUM']).copy()
            if len(match_component) > 0:
                ...
                if ystd_col is not None:
                    ...
                    new_datadf[ystd_col] = row['data'][ystd_col].pow(2).add(match_row[ystd_col].pow(2).mul(scalar**2)).pow(0.5)
    ...
    def check_mirroring(self, field=None, plot=True, plot_x=NM, **kwargs):
        ...
        if field is None:
            ...
            sub_data.info_df = sub_data.info_df.loc[sub_data.info_df[self.labels['FIELD']].isin(field)].copy()
        ...
        for i,row in sub_data.info_df.iterrows():
            ...
            for j,row2 in oppo.iterrows():
                if abs(row2[self.labels['SCAN_NUM']] - row[self.labels['SCAN_NUM']]) <= abs(oppo.at[closest_idx, self.labels['SCAN_NUM']] - row[self.labels['SCAN_NUM']]):
                    if len(oppo) != len(sub_data.info_df.loc[(self.info_df[self.labels['FIELD']]==row[self.labels['FIELD']]) & (self.info_df[self.labels['TEMP']]==row[self.labels['TEMP']])]) or j not in oppo_idxs[oppo_idx_key]: #only allow duplicates if lists not the same size
                        closest_idx = j
    ...
    def half_subd_fields(self, field=None, ystd_col= None, inplace: bool = True):
        ...
        for i,row in sub_data.info_df.iterrows():
            ...
            if len(oppo) > 0:
                ...
                if ystd_col is not None:
                    sub_data.info_df.at[i,'data'][ystd_col] = 0.5 * np.sqrt(np.add(np.square(row['data'][ystd_col]),np.square(oppo['data'][ystd_col])))
                ...
                sub_data.info_df.at[i,self.labels['TEMP_AVG']] = np.mean([row[self.labels['TEMP_AVG']],oppo[self.labels['TEMP_AVG']]])
                sub_data.info_df.at[i,self.labels['FIELD_AVG']] = np.mean(np.abs([row[self.labels['FIELD_AVG']],oppo[self.labels['FIELD_AVG']]]))
                sub_data.info_df.at[i,self.labels['TEMP_DEV']] = 0.5* np.sqrt(np.add(np.square(row[self.labels['TEMP_DEV']]),np.square(oppo[self.labels['TEMP_DEV']])))
                sub_data.info_df.at[i,self.labels['FIELD_DEV']] = 0.5* np.sqrt(np.add(np.square(row[self.labels['FIELD_DEV']]),np.square(oppo[self.labels['FIELD_DEV']])))
                        # 'TEMP_AVG' : 'Avg_Sample_Temp(K)',
                        # 'FIELD_AVG' : 'Avg_Magnet_Field(T)',
                        # 'TEMP_DEV' : 'StdDev_Sample_Temp(K)',
                        # 'FIELD_DEV' : 'StdDev_Magnet_Field(T)',
    ...
    def quick_plot(self, x=None, y=None, x_range=None, y_range=None, ids=None, height=None, width=1000, fig=None,
                   colors=DEFAULT_COLORS, selection_criteria=None,
                   field=None, temp=None, **kwargs):
        ...
        if field is not None and temp is None:
            ...
            selection_criteria = (self.info_df[self.labels['TEMP']].isin(temp)) & (self.info_df[self.labels['FIELD']].isin(field))
        #keyword args so this does not silently break if the parent's parameter order changes
        return super().quick_plot(x=x, y=y, x_range=x_range, y_range=y_range, ids=ids, height=height,
                                  width=width, fig=fig, colors=colors,
                                  selection_criteria=selection_criteria, **kwargs)
    ...
    def add_satmag_x(self):
        ...
        self.info_df['uBHover2kbT'] = 0.5*RecipBoltz*np.divide(np.multiply(zeemanFactor, abs(self.info_df[self.labels['FIELD_AVG']])),
                                                        self.info_df[self.labels['TEMP_AVG']])
        self.info_df['1overkbT'] = RecipBoltz*np.divide(1, self.info_df[self.labels['TEMP_AVG']])
    ...
    def add_satmag_y(self, x_val:float, x_col:str = WAVE, y_col= None, ystd_col = None):
        ...
        for pos, (i, row) in enumerate(self.info_df.iterrows()):
            ...
            nearest = row['data'].loc[row['data'][x_col] > x_val].sort_values(x_col, ascending=True).reset_index()
            if len(nearest) == 0:
                #otherwise this fails with a bare KeyError: 0 several frames down
                raise ValueError(f'{row["id"]} has no {x_col} value above x_val={x_val}; '
        ...
        self.info_df[f'normI_{x_val:.1f}'] = np.divide(abs(self.info_df[f'normI_{x_val:.1f}']), norm_factor)
        self.info_df[f'stdI_{x_val:.1f}'] = np.divide(inten_x_std, norm_factor)

    def make_satmag_fit_csv(self, x_val, x_col:str = WAVE, y_col= None, name : str='mcdfit_data.csv', ystd_col = None, save: bool=True) -> pd.DataFrame:
        ...
        for i,x_vali in enumerate(x_val):
            if f'normI_{x_vali:.1f}' not in self.info_df.columns:
                self.add_satmag_y(x_val=x_vali, x_col=x_col, y_col=y_col, ystd_col=ystd_col)
    ...
    def make_simpoints_csv(self, file_name:str='vtvh_simpoints.csv', nfields:int=100):
        ...
        for i,temp in enumerate(self.info_df[self.labels['TEMP']].unique()):
            avg_temps.append(self.info_df.loc[self.info_df[self.labels['TEMP']] == temp][self.labels['TEMP_AVG']].mean())
    ...
    def plt_satmag(self, x_val: float, iso: str = 'temp', which_iso: list | None = None, x_col_unit: str = '$cm^{-1}$', x_col:str = WAVE, y_col= None, ystd_col = None,
                   line=False, fig=None, ax=None, marker='.', separate=False, sharex = True, ncols=1,
                   colors=PLT_COLORS,
                   fontdict = None, **kwargs) -> Tuple[matplotlib.figure.Figure, matplotlib.axes.Axes]:
        ...
        colors = [colors[j % len(colors)] for j in range(max(len(iso_lines), len(colors)))]
        #set up plot
        if fig is None or ax is None:
            if separate:
                fig,ax = plt.subplots(ncols=ncols, nrows=math.ceil(len(iso_lines)/ncols), figsize=[4*ncols,2.5*len(iso_lines)/ncols], sharex=sharex, squeeze=False)
                ax = ax.flatten()
            else:
        ...
        for j,il in enumerate(iso_lines):
            if which_iso is None or il in which_iso:
                sele_df = self.info_df.loc[abs(self.info_df[iso_col]) == il].copy().sort_values(noniso_col)
                if separate:
                    ax[j].errorbar(sele_df[satmag_x_col],sele_df[f'normI_{x_val:.1f}'],
                        yerr=sele_df[f'stdI_{x_val:.1f}'], label=f'{il} {iso_unit}',
                        linestyle='-' if line else 'none', marker=marker, capsize=2, color=colors[j], **kwargs)
                    ax[j].legend()
                else:
                    ax.errorbar(sele_df[satmag_x_col],sele_df[f'normI_{x_val:.1f}'],
                        yerr=sele_df[f'stdI_{x_val:.1f}'], label=f'{il} {iso_unit}',
                        linestyle='-' if line else 'none', marker=marker, capsize=2, color=colors[j], **kwargs)
    ...
    def calculate_I_MCD(self, g_parallel, delta, A_SatLim, H, T) -> float:
        ...
        def integrand(theta):
            ...
            tanh_term = np.tanh(np.sqrt(delta**2 + (g_parallel * beta * H * cos_theta)**2) / (2 * kT))
    ...
    def calculate_I_MCD_wPolarizations(self,  fitvars: list[float],  
                                    H: float, T: float,
                                    g_perpendicular: float = 1.0, g_parallel=8.0, delta=0, MzoverMxy=0, B0=0,
                                    exponential = False, neg_M =True) -> float:
        r"""
        ...
        def integrand_1(theta: float) -> float:
            ...
            cos_theta = np.cos(theta)
            sin_theta = np.sin(theta)
            gamma = np.sqrt(delta**2 + (g_parallel * beta * H * cos_theta)**2 + (g_perpendicular * beta * H * sin_theta)**2)
            term = (g_parallel * beta * H / gamma) * cos_theta**2 * sin_theta * np.tanh(gamma / (2 * kT))
            return term
        ...
        def integrand_2(theta: float) -> float:
            ...
            sin_theta = np.sin(theta)
            gamma = np.sqrt(delta**2 + (g_parallel * beta * H * cos_theta)**2 + (g_perpendicular * beta * H * sin_theta)**2)
            term = (g_perpendicular * beta * H / gamma) * sin_theta**3 * np.tanh(gamma / (2 * kT))
            return term
    ...
    def resid_doublet_vtvh(self, fitvars, Hs, Ts, mcd_inten, **kwargs):
        ...
        for i in range(len(Hs)):
            ...
            resids[i] = self.calculate_I_MCD_wPolarizations(fitvars, Hs[i], Ts[i], **kwargs) - mcd_inten[i]
        return resids
    
    def fit_doublet_model_vtvh(self, fitvars: list[float], x_val:float, x_col:str = WAVE, bounds=(-1*np.inf,np.inf),gtol=1e-13, ftol=1e-13, xtol=1e-13,max_nfev=None,
                               y_col= None, ystd_col = None, verbose=1,**kwargs):
        ...
        #run fit
        fit = least_squares(self.resid_doublet_vtvh, fitvars, bounds=bounds,
                            args=(abs(self.info_df[self.labels['FIELD_AVG']]).to_numpy(),
                                  self.info_df[self.labels['TEMP_AVG']].to_numpy(),
                                  self.info_df[f'normI_{x_val:.1f}'].to_numpy()),
                            kwargs= kwargs,
                            verbose=verbose, gtol=gtol, ftol=ftol, xtol=xtol, max_nfev=max_nfev)
        return fit
    
    def plot_doublet_model_vtvh(self, fitvars, x_val:float, x_col:str = WAVE, colors=PLT_COLORS,
        ...
        fig, ax = self.plt_satmag(x_val=x_val, iso= 'temp', x_col=x_col, separate=separate, colors=colors, **kwargs)
        ...
        for i,temp in enumerate(iso_lines):
            avg_temp = self.info_df.loc[self.info_df[self.labels['TEMP']] == temp][self.labels['TEMP_AVG']].mean()
            plot_xvals = 0.5*RecipBoltz*np.divide(np.multiply(zeemanFactor, fields), avg_temp)
            # fit_inten = [self.calculate_I_MCD(g_parallel=fitvars[0], delta=fitvars[1], A_SatLim=fitvars[2], H=h, T=temp) for h in fields]
            fit_inten = [self.calculate_I_MCD_wPolarizations(fitvars, H=h, T=avg_temp) for h in fields]

            if separate:
                ax[i].plot(plot_xvals, fit_inten, label=f'{avg_temp} {iso_unit}', c=colors[i])
            else:
                ax.plot(plot_xvals, fit_inten, label=f'{avg_temp} {iso_unit}', c=colors[i])
            # ax.legend()
...
class VTVH_MCD(MCD):
            ...
            if self.info_df.at[i,'File'] in data_files:
                ...
                with open(file_path, 'r') as f:
                    ...
                    for line in f:
                        ...
                        count = count+1
                if npts is None:
                    raise ValueError(f'No NPOINTS value found in {file_path}; is this an interval-scan export?')
                ...
                for yi, channel in enumerate(ylabels):
                    ...
                    channel_df = pd.read_csv(file_path, skiprows=count,
                                             header=0, index_col=False, na_values='1.#INF',
                                             nrows=npts, dtype=float)
                    #rename columns appropriately
                    new_names = [f'{channel}_{k}' if k != 0 else xlabels[0] for k,name in enumerate(list(channel_df.columns))]
                    new_names_dict = {}
                    ...
                    self.info_df.at[i,'data'][list(channel_df.columns)] = channel_df[list(channel_df.columns)]

                    count += npts+2 #increase rows to skip to include the next table of values + "channel" header and data header
    ...
    def _average_ys(self, y_str:str = CD_INTERVAL):
        ...
        cols_to_avg = [col for col in list(self.info_df['data'].iloc[0].columns) if y_str in col]
        ...
        for i in self.info_df.index:
            ...
            self.info_df.at[i,'data'][y_str+'_avg'] = self.info_df.at[i,'data'][cols_to_avg].mean(axis=1)
            self.info_df.at[i,'data'][y_str+'_std'] = self.info_df.at[i,'data'][cols_to_avg].std(axis=1)
    ...
    def average_replicates(self):
        ...
        for t in all_temps:
            for f in all_fields:
                #select all rows with that t and f combo 
                tf_df = self.info_df.loc[(self.info_df[self.labels['TEMP']] == t) & (self.info_df[self.labels['FIELD']] == f)].sort_values(self.labels['SCAN_NUM']).copy()
                if len(tf_df) > 0:
                    ...
                    new_row[self.labels['SCAN_NUM']] = tf_df.at[0,self.labels['SCAN_NUM']]
                    new_row[self.labels['TEMP']] = tf_df.at[0,self.labels['TEMP']]
                    new_row[self.labels['FIELD']] = tf_df.at[0,self.labels['FIELD']]
                    new_row[self.labels['TEMP_AVG']] = tf_df[self.labels['TEMP_AVG']].mean(axis=0)
                    new_row[self.labels['FIELD_AVG']] = tf_df[self.labels['FIELD_AVG']].mean(axis=0)
                    ...
                    new_row[self.labels['TEMP_DEV']] = np.sqrt(np.divide(tf_df[self.labels['TEMP_DEV']].pow(2).mul(tf_df['Num_Temp_Checks'].mean()).sum(),(tf_df['Num_Temp_Checks'].mean()*len(tf_df))) + np.divide(tf_df[self.labels['TEMP_AVG']].sub(tf_df[self.labels['TEMP_AVG']].mean()).pow(2).mul(tf_df['n_replicates'].mean()).sum(),(tf_df['n_replicates'].mean()*len(tf_df))))
                    new_row[self.labels['FIELD_DEV']] = np.sqrt(np.divide(tf_df[self.labels['FIELD_DEV']].pow(2).mul(tf_df['Num_Temp_Checks'].mean()).sum(),(tf_df['Num_Temp_Checks'].mean()*len(tf_df))) + np.divide(tf_df[self.labels['FIELD_AVG']].sub(tf_df[self.labels['FIELD_AVG']].mean()).pow(2).mul(tf_df['n_replicates'].mean()).sum(),(tf_df['n_replicates'].mean()*len(tf_df))))
                    ...
                    new_row['Num_Temp_Checks'] = tf_df['Num_Temp_Checks'].mean()*len(tf_df)
                    #Update the data
                    first_data = self.info_df['data'].iloc[0]
                    cols_to_avg = [col for col in list(first_data.columns) if '_avg' in col]
                    cols_to_std = [col for col in list(first_data.columns) if '_std' in col]
                    # Make a new avg column
                    new_datadf = pd.DataFrame()
                    # Check that NM cols match of if not align the dfs
                    nm_data = pd.DataFrame([tf_df.at[i,'data'][self.labels['NM']] for i in range(len(tf_df))])
                    if not nm_data.mean(axis=0).round(1).equals(tf_df.at[0,'data'][self.labels['NM']].round(1)):
                        ...
                        for i in range(len(tf_df)):
                            tf_df.at[i,'data'].set_index(self.labels['NM'], inplace=True, drop=True)
                            first, new = tf_df.at[0,'data'].align(tf_df.at[i,'data'], join="outer", axis=0)
                            tf_df.at[i,'data'] = new
                    for col in first_data.columns:
                        col_data = pd.DataFrame([tf_df.at[i,'data'][col] for i in range(len(tf_df))])
                        if col in cols_to_avg:
                            ...
                            avgcol = pd.DataFrame([tf_df.at[i,'data'][col.replace('_std','_avg')] for i in range(len(tf_df))])
                            new_datadf[col] = col_data.pow(2).mul(nScansPerPoint).sum(axis=0).div(nScansPerPoint*len(tf_df)) + avgcol.sub(avgcol.mean(axis=0)).pow(2).mul(nScansPerPoint).sum(axis=0).div(nScansPerPoint*len(tf_df))
                            new_datadf[col] = new_datadf[col].pow(0.5)
                        else:
                            if not col_data.mean(axis=0).round(1).equals(tf_df.at[0,'data'][col].round(1)):
                                print(f'Warning: Values in {col} are not all the same. Averaging anyways.')
                    ...
                    # print(new_row)
                    new_info_df = pd.concat([new_info_df, pd.Series(new_row).to_frame().T], ignore_index=True)
