...
class AbsCD(LabData):
            ...
            if self.info_df.at[i,'File'] in data_files:
                ...
                with open(file_path, 'r') as f:
                    ...
                    for line in f:
                        ...
                        count = count+1
                if npts is None:
                    raise ValueError(f'No NPOINTS line found in {file_path}; is this a J-1700 export?')
                #read in csv as dataframe to correct place in info_df
                self.info_df.at[i,'data'] = pd.read_csv(file_path, skiprows=count, header=None,
                                            nrows=npts, names=np.concatenate((xlabels,ylabels)), dtype=float, na_values=['1.#INF'])
        return True
    ...
    def add(self, ref_id, ys, ids_to_subtract=None, square=False):
        ...
        for i, row in self.info_df.iterrows():
            if ids_to_subtract is None or row['id'] in ids_to_subtract: #only subtract from selected samples
                if square:
                    self.info_df.at[i,'data'][ys] = row['data'][ys].pow(2).add(ref_values).pow(0.5)
                else:
    ...
    def fix_changeover(self, ys, x_change, x_col='NANOMETERS', to_move='Less', ignore_idx=None, pts=2):
        ...
        for i, row in self.info_df.iterrows():
            if ignore_idx is None or i not in ignore_idx:
                ...
                before = np.average(self.info_df.at[i,'data'].loc[self.info_df.at[i,'data'][x_col] < x_change].head(pts)[ys].values, axis=0)
                after = np.average(self.info_df.at[i,'data'].loc[self.info_df.at[i,'data'][x_col] >= x_change].tail(pts)[ys].values, axis=0)
                ...
                if to_move == 'Less' or to_move== 'less':
                    ...
                    self.info_df.at[i,'data'][ys] = pd.concat((self.info_df.at[i,'data'].loc[self.info_df.at[i,'data'][x_col] < x_change][ys].add(diff/2),
                                                                self.info_df.at[i,'data'].loc[self.info_df.at[i,'data'][x_col] >= x_change][ys].sub(diff/2)))
    ...
    def add_eV(self, nm_str='NANOMETERS'):
        ...
        if nm_str in self.info_df['data'].iloc[0].columns:
            for i,row in self.info_df.iterrows():
                self.info_df.at[i,'data']['eV'] = np.divide(1239.8, row['data'][nm_str].astype(float))
    ...
    def _conc_to_M(conc, conc_units):
        ...
        if conc_units not in factors:
            raise ValueError(f'conc_units must be one of {sorted(factors)}, got {conc_units!r}.')
    ...
    def add_deps(self, conc, conc_units='M', path_length=1):
        ...
        if CD in self.info_df['data'].iloc[0].columns:
            ...
            for pos, (i, row) in enumerate(self.info_df.iterrows()):
                self.info_df.at[i,'data']['deps'] = np.divide(row['data'][CD],(concM[pos] * path_length * 32980))
        print(self.info_df['data'].iloc[0].columns.values)
        return True
        ...
        if abs_str in self.info_df['data'].iloc[0].columns:
            ...
            for pos, (i, row) in enumerate(self.info_df.iterrows()):
                if ids is None or row['id'] in ids:
                    self.info_df.at[i,'data']['eps'] = np.divide(row['data'][abs_str],(concM[pos] * path_length))
        print(self.info_df['data'].iloc[0].columns.values)
        return True
    ...
    def fit_gaussians(self, energies, fwhm, intens, id, x_col=WAVE, y_cols=('eps','deps'), xrange=None, low_bds=None, up_bds=None, same_x=True, gtol=1e-13, ftol=1e-13, xtol=1e-13, scalar=None):
        ...
        ys = [row_data[y_cols].to_numpy()] if isinstance(y_cols, str) else row_data[list(y_cols)].T.to_numpy()
    ...
    def check_plot(self, id:str, result=None, x_col=WAVE, y_cols=('eps','deps'), xrange=None, *args, **kwargs):
        ...
        **kwargs is passed through to quick_plot'''
        ...
        for j, row_traces in enumerate(traces_by_y):
            for lab, values in row_traces:
                fig.add_trace(go.Scatter(x=xs, y=values, name=lab, mode='lines',
