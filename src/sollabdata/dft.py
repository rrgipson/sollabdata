...
class DFT(LabData):
    ...
    def read(self, path_to_raw_data, info_csv):
        ...
        if info_path is not None:
            ...
            self.info_df = pd.DataFrame(
                {"id": [os.path.splitext(f)[0] for f in data_files], "File": data_files}
            )
    ...
    def load(self, path_to_raw_data, data_files, **kwargs):
        ...
        for i, row in self.info_df.iterrows():
            if self.info_df.at[i, "File"] in data_files:
                self.info_df.at[i, "data"] = pd.read_json(
                    os.path.join(path_to_raw_data, row["File"]), **kwargs
                )
        return True
    ...
    def get_atoms(self, id, df=None, file_ind=None) -> pd.DataFrame:
        ...
        atom_list = [
            ATOM_NAMES.get(num, "ATOM NOT FOUND")
            for num in self.get_param(id, "atomnos", file_ind)
        ]
        if df is None:
    ...
    def get_opt_geom(self, id, df, file_ind=None) -> pd.DataFrame:
        ...
        df_temp = pd.DataFrame(
            data=self.get_param(id, "atomcoords", file_ind)[-1], columns=["x", "y", "z"]
        )
    ...
    def get_thermo(
        ...
        thermo_data = {}
        if not skipscf:
            thermo_data["E"] = convertor(
                self.get_param(id, "scfenergies", freq_idx)[-1], "eV", units
            )
            thermo_data["Ezpve"] = thermo_data["E"] + convertor(
                self.get_param(id, "zpve", freq_idx), "hartree", units
            )
        thermo_data["H"] = convertor(
            self.get_param(id, "enthalpy", freq_idx), "hartree", units
        )
        thermo_data["S"] = convertor(
            self.get_param(id, "entropy", freq_idx), "hartree", units
        )
        thermo_data["G"] = convertor(
            self.get_param(id, "freeenergy", freq_idx), "hartree", units
        )
        ...
        if reactant_id is not None:
            ...
            if thermo_data["T_K"] != reactant.get_param(
                reactant_id, "temperature", react_freq_idx
            ):
                print(
                    "Warning: Different temperatures between reactant and product data."
                )
                thermo_data["T_K_reactant"] = reactant.get_param(
                    reactant_id, "temperature", react_freq_idx
                )
            if not skipscf:
                thermo_data["E"] -= convertor(
                    reactant.get_param(reactant_id, "scfenergies", react_freq_idx)[-1],
                    "eV",
                    units,
                )
                thermo_data["Ezpve"] -= convertor(
                    reactant.get_param(reactant_id, "scfenergies", react_freq_idx)[-1],
                    "eV",
                    units,
                ) + convertor(
                    reactant.get_param(reactant_id, "zpve", react_freq_idx),
                    "hartree",
                    units,
                )
            thermo_data["H"] -= convertor(
                reactant.get_param(reactant_id, "enthalpy", react_freq_idx),
                "hartree",
                units,
            )
            thermo_data["S"] -= convertor(
                reactant.get_param(reactant_id, "entropy", react_freq_idx),
                "hartree",
                units,
            )
            thermo_data["G"] -= convertor(
                reactant.get_param(reactant_id, "freeenergy", react_freq_idx),
                "hartree",
                units,
            )
    ...
    def get_scf(
        ...
        if reactant_id is not None:
            reactant = self if reactant is None else reactant
            scf -= convertor(
                reactant.get_param(reactant_id, "scfenergies", react_file_ind)[-1],
                "eV",
                units,
            )
    ...
    def get_dist(self, id, a1, a2, a3=None, file_ind=None, verbose=False):
        ...
        if verbose:
            print(
                dist_txt.format(
                    start=df.at[a1 - 1, "Atom"], stop=df.at[a2 - 1, "Atom"], dist=dist12
                )
            )
        if a3 is not None:
            # get a2-a3 distance
            ...
            if verbose:
                print(
                    dist_txt.format(
                        start=df.at[a2 - 1, "Atom"],
                        stop=df.at[a3 - 1, "Atom"],
                        dist=dist23,
                    )
                )
            # get a1-a2-a3 angle
    ...
    def get_dihedral(self, id, a1, a2, a3, a4, file_ind=None, verbose=False):
        ...
        if verbose:
            ...
            print(
                dihedral_txt.format(
                    a1=atom1, a2=atom2, a3=atom3, a4=atom4, angle=dihedral
                )
            )
